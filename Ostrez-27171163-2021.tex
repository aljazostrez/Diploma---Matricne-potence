...
\povzetek{V par stavkih poveš, kaj je bistvo te naloge (ne rezultatov, ampak s čem se ukvarja ta naloga)}
...
\end{figure}
Kolikšen delež svojega časa nadzornik preživi v mestih $A,B,C,D,E$ po dolgem času?
...
\end{align*}
Vse lastne vrednosti razen $\lambda_1$ so po absolutni vrednosti strogo manjši od $1$, torej po izreku \ref{izrekAsimptotika} sledi, da je zaporedje $(P^k)_{k\in\N}$ konvergentno. Iz tega sledi, da je konvergentno tudi zaporedje Ces\`arovih poprečij $\Big(\frac{1}{k} \sum_{l=0}^{k-1} P^l\Big)_{k\in\N}$.

Recimo, da je nadzornik na začetku v mestu $A$. Porazdelitveni vektor na $0$-tem koraku bo torej enak $p(0) = (1, 0, 0, 0, 0)^T$. Po formuli \eqref{eqKtiKorakMarkov} lahko izračunamo porazdelitveni vektor na $k$-tem koraku
\begin{equation*}
    p(k) = \left(P^k\right)^T p(0)
\end{equation*}
za vsak $k\in\N$.
\textbf{Tukaj bi potreboval nekoliko pomoči. Kaj točno gledamo, ko se vprašamo, koliko časa nadzornik preživi v nekem mestu? Cesarova povprečja? Ampak to je matrika. NAVADNO POVPREČJE - SEŠTEJEŠ VEKTORJE IN DELIŠ S k.}

\textbf{Vključi ali premisli, če bi zgornji zgled zamenjal s shell game ali pa ta zgled malo prilagodim. Ali pa dodam, kaj bi se zgodilo, če kakšna povezava (C-E) ne bi bila dvosmerna.}
